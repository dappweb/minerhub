...
let announcementSchemaInit = null;
async function ensureAnnouncementsSchema(env) {
    if (announcementSchemaInit)
    ...
    announcementSchemaInit = (async () => {
        ...
        const columnInfo = await env.DB.prepare("PRAGMA table_info(announcements)").all();
        const columns = new Set((columnInfo.results ?? []).map((item) => item.name));
        const alterStatements = [];
        if (!columns.has("title_en"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN title_en TEXT");
        if (!columns.has("content_en"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN content_en TEXT");
        if (!columns.has("level"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN level TEXT NOT NULL DEFAULT 'info'");
        if (!columns.has("target"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN target TEXT NOT NULL DEFAULT 'all'");
        if (!columns.has("is_pinned"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN is_pinned INTEGER NOT NULL DEFAULT 0");
        if (!columns.has("publish_at"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN publish_at TEXT");
        if (!columns.has("expire_at"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN expire_at TEXT");
        if (!columns.has("created_by"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN created_by TEXT");
        if (!columns.has("created_at"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN created_at TEXT");
        if (!columns.has("updated_at"))
            alterStatements.push("ALTER TABLE announcements ADD COLUMN updated_at TEXT");
        for (const statement of alterStatements) {
            await env.DB.prepare(statement).run();
        }
        const now = nowIso();
        await env.DB.prepare(`UPDATE announcements
       SET title_en = COALESCE(NULLIF(title_en, ''), title_zh),
           ...
           updated_at = COALESCE(NULLIF(updated_at, ''), created_at, publish_at, ?)`)
            .bind(now, now)
            .run();
        const readsInfo = await env.DB.prepare("PRAGMA table_info(announcement_reads)").all();
...
async function listPublicAnnouncements(env) {
    const now = nowIso();
    const { results } = await env.DB.prepare(`SELECT id, title_zh, title_en, content_zh, content_en, level, target,
...
async function createAnnouncement(request, env) {
    ...
    await env.DB.prepare(`INSERT INTO announcements (
      id, title_zh, title_en, content_zh, content_en, level, target,
      is_published, is_pinned, publish_at, expire_at, created_by, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(id, titleZh, titleEn || titleZh, contentZh, contentEn || contentZh, normalizeLevel(body.level), normalizeTarget(body.target), isPublished ? 1 : 0, body.isPinned ? 1 : 0, publishAt, expireAt, wallet, now, now)
        .run();
    const created = await env.DB.prepare(`SELECT id, title_zh, title_en, content_zh, content_en, level, target,
            is_published, is_pinned, publish_at, expire_at, created_by, created_at, updated_at
...
async function updateAnnouncement(request, env, announcementId) {
    ...
    const current = await env.DB.prepare(`SELECT id, title_zh, title_en, content_zh, content_en, level, target,
            is_published, is_pinned, publish_at, expire_at, created_by, created_at, updated_at
     FROM announcements WHERE id = ?`)
        .bind(announcementId)
        .first();
    ...
    await env.DB.prepare(`UPDATE announcements
     SET title_zh = ?,
         ...
         updated_at = ?
     WHERE id = ?`)
        .bind(titleZh ?? current.title_zh, titleEn ?? current.title_en, contentZh ?? current.content_zh, contentEn ?? current.content_en, normalizeLevel(body.level ?? current.level), normalizeTarget(body.target ?? current.target), typeof body.isPublished === "boolean" ? (body.isPublished ? 1 : 0) : current.is_published, typeof body.isPinned === "boolean" ? (body.isPinned ? 1 : 0) : current.is_pinned, nextPublishAt, nextExpireAt, nowIso(), announcementId)
        .run();
    const updated = await env.DB.prepare(`SELECT id, title_zh, title_en, content_zh, content_en, level, target,
            is_published, is_pinned, publish_at, expire_at, created_by, created_at, updated_at
     FROM announcements WHERE id = ?`)
        .bind(announcementId)
        .first();
...
async function publishAnnouncement(env, announcementId, isPublished) {
...
async function deleteAnnouncement(env, announcementId) {
...
async function markAnnouncementRead(request, env, userId, announcementId) {
    ...
    if (!announcement)
        return json({ error: "Announcement not found" }, 404);
    const readAt = nowIso();
    await env.DB.prepare(`INSERT INTO announcement_reads (announcement_id, user_id, wallet, read_at)
     VALUES (?, ?, ?, ?)
...
export async function handleAnnouncements(request, env, pathParts) {
    ...
    if (pathParts[0] === "admin") {
        ...
        if (adminParts.length === 1) {
            if (request.method === "PUT")
                return updateAnnouncement(request, env, adminParts[0]);
            if (request.method === "DELETE")
                return deleteAnnouncement(env, adminParts[0]);
        }
