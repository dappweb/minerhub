...
export async function reconcileUserRewardTotals(env, userId, atIso = nowIso()) {
    const earned = await env.DB.prepare(`SELECT
    ...
    const withdrawn = await env.DB.prepare(`SELECT COALESCE(SUM(amount_super), 0) AS amount_super
     FROM reward_withdrawals
     WHERE user_id = ? AND status = 'confirmed'`)
        .bind(userId)
        .first();
    const statusPlaceholders = RESERVED_EXCHANGE_STATUSES.map(() => "?").join(", ");
    const exchanged = await env.DB.prepare(`SELECT COALESCE(SUM(CAST(amount_super AS REAL)), 0) AS amount_super
    ...
    const totalRewardSuper = round6(Number(earned?.total_super ?? 0)
