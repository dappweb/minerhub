...
export default {
    async fetch(request, env) {
        ...
        try {
            ...
            if (scope === "system")
                return await handleSystem(request, env, pathParts);
            if (scope === "announcements")
                return await handleAnnouncements(request, env, pathParts);
            if (scope === "admin")
                return await handleAdmin(request, env, pathParts);
            if (scope === "owner")
                return await handleOwner(request, env, pathParts);
            if (scope === "users")
                return await handleUsers(request, env, pathParts);
            if (scope === "referrals")
                return await handleReferrals(request, env, pathParts);
            if (scope === "devices")
                return await handleDevices(request, env, pathParts);
            if (scope === "claims")
                return await handleClaims(request, env, pathParts);
            if (scope === "downloads")
                return await handleDownloads(request, env, pathParts);
            if (scope === "gas")
                return await handleGas(request, env, pathParts);
            if (scope === "operations")
                return await handleOperations(request, env, pathParts);
            if (scope === "rpc")
                return await handleRpc(request, env, pathParts);
