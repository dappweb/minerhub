...
async function listDownloads(env) {
    ...
    const androidMeta = androidMetaRaw
        ? JSON.parse(androidMetaRaw)
        : null;
    return json({
        android: androidMeta
            ? {
...
async function downloadAndroid(env) {
    if (!env.APP_BUCKET) {
...
async function downloadIos(env) {
...
// admin handlers
// ──────────────────────────────────────────
async function uploadAndroid(request, env) {
    ...
    if (contentType.includes("multipart/form-data")) {
        ...
        version = formData.get("version") ?? version;
    }
    else {
        // raw binary body
...
async function updateIosUrl(request, env) {
    if (!(await isOwner(request, env))) {
        return unauthorized("Owner wallet required");
    }
    let body;
...
async function deleteAndroid(request, env) {
    if (!(await isOwner(request, env))) {
        return unauthorized("Owner wallet required");
    }
    if (!env.APP_BUCKET) {
...
// router
// ──────────────────────────────────────────
export async function handleDownloads(request, env, pathParts) {
    ...
    try {
        ...
        if (platform === "android") {
            if (request.method === "GET")
                return downloadAndroid(env);
            if (request.method === "PUT")
                return uploadAndroid(request, env);
            if (request.method === "DELETE")
                return deleteAndroid(request, env);
        }
        if (platform === "ios") {
            if (request.method === "GET")
                return downloadIos(env);
            if (request.method === "PUT")
                return updateIosUrl(request, env);
        }
