...
export async function ensureContractAccessColumns(env) {
    if (columnsReady)
        return;
    const [profileInfo, subAdminInfo] = await Promise.all([
        env.DB.prepare("PRAGMA table_info(customer_profiles)").all(),
        env.DB.prepare("PRAGMA table_info(owner_sub_admins)").all().catch(() => ({ results: [] })),
    ]);
...
export function normalizeContractTypes(value) {
...
export function parseAllowedContractTypes(raw) {
    if (!raw)
        return [];
    try {
...
export function serializeAllowedContractTypes(types) {
    return JSON.stringify(normalizeContractTypes(types));
}
export function contractTypeFromTerm(termDays) {
    if (!Number.isFinite(Number(termDays)))
        return null;
...
export function contractTypeFromYears(years) {
...
export function contractTypesEqual(a, b) {
    const left = normalizeContractTypes(a).sort();
    const right = normalizeContractTypes(b).sort();
    return left.length === right.length && left.every((item, index) => item === right[index]);
}
export function builtInContractTypeIds() {
    return Array.from(BUILTIN_IDS);
}
export function addContractScopeClause(clauses, params, allowedTypes, profileAlias = "cp") {
    ...
    clauses.push(`(${profileAlias}.contract_type IS NULL OR TRIM(${profileAlias}.contract_type) = '' OR ${profileAlias}.contract_type IN (${allowedTypes.map(() => "?").join(",")}))`);
    params.push(...allowedTypes);
}
export async function getSubAdminContractScope(env, wallet) {
    if (!wallet)
        return [];
    await ensureContractAccessColumns(env);
    let row = null;
    try {
        row = await env.DB.prepare(`SELECT allowed_contract_types_json
...
export async function readCustomerContractType(env, userId) {
    await ensureContractAccessColumns(env);
...
export function isContractTypeInScope(allowedTypes, contractType) {
    if (allowedTypes === null)
        return true;
    if (!contractType)
        return true;
...
export async function canAccessCustomerContractType(env, allowedTypes, userId) {
    return isContractTypeInScope(allowedTypes, await readCustomerContractType(env, userId));
}
export async function resolveServiceContractType(env, allowedTypes, userId, requestedType) {
...
export async function setCustomerContractTypeIfEmpty(env, userId, contractType) {
