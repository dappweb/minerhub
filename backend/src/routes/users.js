...
function deriveLiveOnlineStatus(lastSeenAt) {
    if (!lastSeenAt)
        return "offline";
...
async function ensureReferrerUser(env, referralWalletRaw) {
...
async function bindReferralRelation(env, inviteeId, inviteeWallet, referralWalletRaw) {
    const referralWallet = referralWalletRaw.trim().toLowerCase();
    if (!referralWallet)
        return;
    if (inviteeWallet.toLowerCase() === referralWallet)
        return;
    ...
    const ancestors = await env.DB.prepare(`SELECT ancestor_user_id, depth FROM referral_closure WHERE descendant_user_id = ?`)
        .bind(inviter.id)
        .all();
    const descendantChain = [{ id: inviteeId, depthFromInvitee: 0 }];
    const ancestorChain = [
...
export async function handleUsers(request, env, pathParts) {
    if (request.method === "POST" && pathParts.length === 0) {
        ...
        const now = nowIso();
        try {
            await env.DB.prepare("INSERT INTO users (id, wallet, email, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
    ...
    if (request.method === "GET" && pathParts.length === 2 && pathParts[1] === "details") {
      ...
      WHERE u.id = ?`)
        ...
        if (!user)
            return json({ error: "User not found" }, 404);
        const devices = await env.DB.prepare("SELECT id, device_id, hashrate, status, created_at, updated_at FROM devices WHERE user_id = ? ORDER BY created_at DESC")
            .bind(userId)
            .all();
        const rewards = await env.DB.prepare("SELECT id, device_id, reward_usdt, reward_super, rate_usdt_per_hour, source, note, created_at, updated_at FROM reward_ledger WHERE user_id = ? ORDER BY created_at DESC LIMIT 50")
            .bind(userId)
            .all();
        const wallets = await env.DB.prepare("SELECT wallet_address, priority, is_primary FROM payout_wallets WHERE user_id = ? ORDER BY priority ASC, created_at ASC")
            .bind(userId)
            .all();
        const acceptance = await env.DB.prepare("SELECT version, accepted_at FROM user_agreement_acceptances WHERE user_id = ? ORDER BY accepted_at DESC LIMIT 1")
            .bind(userId)
            .first();
        const lockSummary = await env.DB.prepare(`SELECT
         COUNT(*) AS total,
       ...
       FROM token_locks WHERE user_id = ?`)
        ...
        return json({
            ...
            agreementAcceptedAt: acceptance?.accepted_at ?? user.agreementAcceptedAt ?? null,
            contractAgreementAcceptedVersion: user.contract_agreement_accepted_version ?? null,
            lockSummary: {
    ...
    if (request.method === "POST" && pathParts.length === 2 && pathParts[1] === "agreement") {
        ...
        await env.DB.prepare(`INSERT INTO user_agreement_acceptances (user_id, version, accepted_at, wallet)
       VALUES (?, ?, ?, ?)
       ON CONFLICT(user_id, version) DO UPDATE SET accepted_at = excluded.accepted_at, wallet = excluded.wallet`)
            ...
            .run();
        await env.DB.prepare("UPDATE customer_profiles SET agreement_accepted_at = ?, updated_at = ? WHERE user_id = ?")
            .bind(now, now, userId)
            .run();
    ...
    if (request.method === "POST" && pathParts.length === 2 && pathParts[1] === "contract-agreement") {
        ...
        const now = nowIso();
        const version = body.version.trim();
        await env.DB.prepare("UPDATE customer_profiles SET contract_agreement_accepted_version = ?, updated_at = ? WHERE user_id = ?")
    ...
    if (request.method === "GET" && pathParts.length === 0) {
        ...
        if (!wallet)
            return json({ error: "wallet query param is required" }, 400);
