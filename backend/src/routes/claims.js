    ...
    if (request.method === "POST" && pathParts.length === 1 && pathParts[0] === "exchange-request") {
        ...
        try {
          ...
          request_note, created_at, updated_at
                ...
                .bind(exchangeId, body.userId, body.wallet.toLowerCase(), amountSuper.toString(), Number.isFinite(amountUsdt) ? Math.max(0, amountUsdt).toString() : "0", mode, status, body.note?.trim() || null, now, now)
                .run();
          ...
          price_snapshot, status, note, created_at, updated_at
                ...
                .bind(createId("swl"), body.userId, body.wallet.toLowerCase(), amountSuper.toString(), Number.isFinite(amountUsdt) ? Math.max(0, amountUsdt).toString() : "0", status, autoEnabled ? "auto exchange request" : "manual exchange request", now, now)
                .run();
            const totals = await reconcileUserRewardTotals(env, body.userId, now);
            return json({
                id: exchangeId,
