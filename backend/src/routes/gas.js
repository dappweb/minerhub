...
function parsePayToken(value) {
    if (typeof value !== 'string')
        return null;
    const upper = value.toUpperCase();
    if (upper === 'SUPER' || upper === 'USDT')
        return upper;
    return null;
}
...
function getTokenUsdPrice(token, pricing) {
    return token === 'SUPER' ? pricing.superUsd : pricing.usdtUsd;
}
async function transferBnbFromTreasury(env, to, amountBnb) {
...
async function updateWalletCredit(env, wallet, fundedBnb) {
    const now = nowIso();
    await env.DB.prepare(`INSERT INTO gas_wallet_credits (wallet, total_bnb_funded, total_orders, updated_at)
     VALUES (?, ?, 1, ?)
...
async function handleQuote(request, env) {
    ...
    await env.DB.prepare(`INSERT INTO gas_quotes (id, wallet, pay_token, pay_amount, bnb_amount, fee_rate, price_snapshot, expires_at, used, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)`)
        .bind(quoteId, body.wallet.toLowerCase(), payToken, body.payAmount, bnbAmount.toFixed(8), pricing.feeRate.toString(), priceSnapshot, expiresAt, createdAt)
        .run();
    return json({
        quoteId,
...
async function handlePurchase(request, env) {
    ...
    const quote = await env.DB.prepare('SELECT id, wallet, pay_token, pay_amount, bnb_amount, expires_at, used FROM gas_quotes WHERE id = ?')
        .bind(body.quoteId)
        .first();
    if (!quote) {
    ...
    const existingOrder = await env.DB.prepare(`SELECT id, quote_id, wallet, status, relay_mode, relay_tx_hash, error_message, bnb_amount
    ...
    await env.DB.prepare(`INSERT INTO gas_orders (
      ...
      status, relay_mode, relay_tx_hash, error_message, created_at, updated_at
        ...
        .bind(orderId, quote.id, body.wallet.toLowerCase(), body.userId ?? null, quote.pay_token, quote.pay_amount, quote.bnb_amount, status, transferResult.mode, transferResult.txHash ?? null, transferResult.errorMessage ?? null, now, now)
        .run();
...
async function handleOrderGet(env, orderId) {
    const order = await env.DB.prepare(`SELECT id, quote_id, wallet, user_id, pay_token, pay_amount, bnb_amount, status,
...
async function handleWalletCredit(request, env) {
    const wallet = new URL(request.url).searchParams.get('wallet');
    if (!wallet) {
        return badRequest('wallet query param is required');
    }
    const row = await env.DB.prepare('SELECT wallet, total_bnb_funded, total_orders, updated_at FROM gas_wallet_credits WHERE wallet = ?')
...
async function handleIntentCreate(request, env) {
    if (await isMaintenanceEnabled(env)) {
        return json({ error: 'System is under maintenance' }, 503);
    }
    const auth = await extractAndVerifyAuth(request, env);
    ...
    const now = nowIso();
    await env.DB.prepare(`INSERT INTO gas_intents (id, wallet, user_id, pay_token, max_token_spend, action, action_payload, status, relay_order_id, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(intentId, body.wallet.toLowerCase(), body.userId ?? null, payToken, body.maxTokenSpend, body.action, JSON.stringify(body.actionPayload ?? {}), 'pending', null, now, now)
        .run();
    return json({
        intentId,
        status: 'pending',
...
async function handleIntentRelay(request, env) {
    if (await isMaintenanceEnabled(env)) {
        return json({ error: 'System is under maintenance' }, 503);
    }
    const auth = await extractAndVerifyAuth(request, env);
    ...
    const intent = await env.DB.prepare('SELECT id, wallet, status FROM gas_intents WHERE id = ?')
        .bind(body.intentId)
        .first();
    if (!intent) {
...
async function handleIntentGet(env, intentId) {
    const intent = await env.DB.prepare(`SELECT id, wallet, user_id, pay_token, max_token_spend, action, action_payload,
...
export async function handleGas(request, env, pathParts) {
