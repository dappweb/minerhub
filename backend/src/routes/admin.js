...
import { addContractScopeClause, canAccessCustomerContractType, contractTypeFromTerm, contractTypeFromYears, ensureContractAccessColumns, getSubAdminContractScope, normalizeContractType, resolveServiceContractType, setCustomerContractTypeIfEmpty, } from "../lib/contractAccess";
import { getAdminActorRole, requireOwnerAuth } from "../lib/ownerAuth";
...
async function ensureCollectionRequestsTable(env) {
    if (collectionRequestsReady)
        return;
    await env.DB.prepare(`CREATE TABLE IF NOT EXISTS fund_collection_requests (
      id TEXT PRIMARY KEY,
...
async function ensureExchangeOrderTxColumns(env) {
    if (exchangeOrderTxColumnsReady)
        return;
...
function addDaysFromActiveEnd(endAt, days, nowMs) {
...
export function deriveLiveOnlineStatus(lastSeenAt) {
    if (!lastSeenAt)
        return "offline";
    ...
    if (diff <= HEARTBEAT_ONLINE_MS)
        return "online";
    if (diff <= HEARTBEAT_STALE_MS)
        return "stale";
    return "offline";
}
async function requireOwnerRead(request, env) {
...
async function ensureUserIdByWallet(env, wallet) {
    ...
    const now = nowIso();
    await env.DB.prepare(`INSERT OR IGNORE INTO users (id, wallet, email, role, created_at, updated_at)
     VALUES (?, ?, NULL, 'subadmin', ?, ?)`)
        .bind(id, normalized, now, now)
        .run();
    return (await findUserIdByWallet(env, normalized)) ?? id;
}
async function canManageUserByScope(env, scopeUserId, targetUserId) {
    if (!scopeUserId)
        return true;
...
async function canAccessUserByScope(env, scopeUserId, allowedTypes, targetUserId) {
    if (!(await canManageUserByScope(env, scopeUserId, targetUserId)))
        return false;
    return canAccessCustomerContractType(env, allowedTypes, targetUserId);
}
async function canManageDeviceByScope(env, scopeUserId, deviceRecordId) {
    if (!scopeUserId)
        return true;
    const row = await env.DB.prepare(`SELECT 1 AS ok
     FROM devices d
...
async function canAccessDeviceByScope(env, scopeUserId, allowedTypes, deviceRecordId) {
    if (!(await canManageDeviceByScope(env, scopeUserId, deviceRecordId)))
        return false;
    if (allowedTypes === null)
        return true;
    const row = await env.DB.prepare(`SELECT d.user_id AS user_id
     FROM devices d
     WHERE d.id = ?
     LIMIT 1`)
        .bind(deviceRecordId)
...
function parsePayoutWallets(raw) {
    if (!raw)
        return [];
    try {
...
async function readCustomerSummariesByInviterWallet(env, inviterWallet, allowedTypes, scopeUserId = null) {
...
async function attachDevicesToCustomerRows(env, rows) {
    if (rows.length === 0)
        return [];
    ...
    return rows.map((row) => {
        ...
        const state = deriveContractState({
            ...
            deviceCount: Number(row.deviceCount ?? devices.length),
        });
        return {
            ...row,
...
async function readDeviceSummariesByUserIds(env, userIds) {
...
async function getCustomerDetail(env, userId) {
    ...
    if (!customer)
        return null;
    const devices = await env.DB.prepare(`SELECT id, device_id, hashrate, status, created_at, updated_at
     FROM devices WHERE user_id = ? ORDER BY created_at DESC`)
        .bind(userId)
        .all();
    const subAccounts = { results: [] };
    const rewardLedger = await env.DB.prepare(`SELECT id, device_id, reward_usdt, reward_super, rate_usdt_per_hour, source, note, created_at, updated_at
     FROM reward_ledger WHERE user_id = ? ORDER BY created_at DESC LIMIT 100`)
        .bind(userId)
        .all();
    const walletRows = await env.DB.prepare(`SELECT wallet_address, priority, is_primary FROM payout_wallets WHERE user_id = ? ORDER BY priority ASC, created_at ASC`)
        .bind(userId)
        .all();
    const summary = await env.DB.prepare(`SELECT COUNT(DISTINCT id) AS device_count, SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) AS active_device_count
     FROM devices WHERE user_id = ?`)
        .bind(userId)
        .first();
    const lastSeenRaw = customer.lastSeenAt ?? null;
    ...
    return {
        ...
        payoutWallets: parsePayoutWallets(customer.payoutWalletsJson ?? null),
...
async function updateProfileField(env, userId, key, value) {
    const now = nowIso();
    await env.DB.prepare(`INSERT INTO customer_profiles (user_id, created_at, updated_at)
     VALUES (?, ?, ?)
...
function calculateContractEnd(startAt, termDays) {
    return new Date(new Date(startAt).getTime() + termDays * 24 * 60 * 60 * 1000).toISOString();
}
async function applyServiceContractType(env, allowedTypes, userId, requestedType) {
...
async function handleCustomerList(request, env, requesterWallet, role, scopeUserId, allowedTypes) {
...
async function readAdminDevices(env, url, scopeUserId, allowedTypes) {
...
async function getAdminDeviceDetail(env, deviceRecordId) {
    ...
    const [historyRows, rewardRows] = await Promise.all([
        env.DB.prepare(`SELECT id, status, hashrate, observed_at, note
       FROM device_status_history
       WHERE user_id = ? AND device_id = ?
       ORDER BY observed_at DESC LIMIT 100`)
            .bind(item.userId, item.deviceId)
            .all(),
        env.DB.prepare(`SELECT id, reward_usdt, reward_super, rate_usdt_per_hour, source, note, created_at
       FROM reward_ledger
    ...
    return {
        ...
        contractActive: Number(item.contractActive ?? 0),
        hashrate: Number(item.hashrate ?? 0),
        monthlyCardDays: Number(item.monthlyCardDays ?? 30),
        onlineStatus: deriveLiveOnlineStatus(item.lastSeenAt ?? null),
        bnbBalance: balances.bnb,
        usdtBalance: balances.usdt,
        superBalance: balances.super,
...
async function handleAdminDeviceList(request, env, scopeUserId, allowedTypes) {
    const data = await readAdminDevices(env, new URL(request.url), scopeUserId, allowedTypes);
    return json(data);
}
async function handleAdminDeviceDetail(env, deviceRecordId) {
    const detail = await getAdminDeviceDetail(env, deviceRecordId);
    if (!detail)
        return json({ error: "Device not found" }, 404);
    return json(detail);
}
async function handleAdminDeviceUpdate(request, env, deviceRecordId, allowedTypes) {
    ...
    const touchesService = allowedTypes !== null &&
...
async function handleAdminDeviceDelete(env, deviceRecordId) {
    ...
    await env.DB.prepare("DELETE FROM devices WHERE id = ?")
        .bind(deviceRecordId)
        .run();
    const summary = await env.DB.prepare("SELECT COUNT(*) AS device_count, SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) AS active_device_count FROM devices WHERE user_id = ?")
...
async function handleAdminDeviceBulkUpdate(request, env, scopeUserId, allowedTypes) {
    const body = (await request.json().catch(() => null));
...
async function handleCustomerDetail(env, userId) {
...
async function handleCustomerUpdate(request, env, userId, allowedTypes) {
    const body = (await request.json().catch(() => null));
    if (!body)
        return badRequest("Invalid JSON body");
    await ensureProfile(env, userId);
    ...
    if (Array.isArray(body.devices)) {
        ...
        for (const item of body.devices) {
            ...
            if (!deviceRecordId)
                return badRequest("devices[].id is required");
            if (seenIds.has(deviceRecordId))
                return badRequest("devices[].id must be unique");
            seenIds.add(deviceRecordId);
            if (!deviceId)
                return badRequest("devices[].deviceId is required");
            if (!status)
                return badRequest("devices[].status is required");
            if (!Number.isFinite(hashrate) || hashrate < 0)
                return badRequest("devices[].hashrate must be >= 0");
            ...
            await env.DB.prepare(`UPDATE devices
    ...
    if (Array.isArray(body.payoutWallets)) {
        const normalized = body.payoutWallets
            ...
            if (item && typeof item === "object" && typeof item.walletAddress === "string") {
                return {
        ...
        for (const wallet of normalized) {
            await env.DB.prepare(`INSERT INTO payout_wallets (id, user_id, wallet_address, priority, is_primary, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
...
async function handleCustomerDelete(env, userId) {
...
async function handleCustomerActivate(request, env, userId, allowedTypes) {
    const body = (await request.json().catch(() => null));
    if (!body)
        return badRequest("Invalid JSON body");
    await ensureProfile(env, userId);
    ...
    const termDays = Number.isFinite(body.contractTermDays ?? NaN)
        ...
        : Number.isFinite(body.contractTermYears ?? NaN)
            ? Math.max(1, Math.floor(body.contractTermYears * 365))
            : 1095;
    const requestedType = normalizeContractType(body.contractType)
...
async function handleRewardAdjustment(request, env, userId, allowedTypes) {
    const body = (await request.json().catch(() => null));
    if (!body)
        return badRequest("Invalid JSON body");
    ...
    await env.DB.prepare(`INSERT INTO reward_ledger (
      ...
      accrued_from, accrued_to, source, note, created_at, updated_at
        ...
        .bind(id, userId, body.deviceId ?? null, String(Number.isFinite(rewardUsdt) ? rewardUsdt : 0), String(Number.isFinite(rewardSuper) ? rewardSuper : 0), String(body.rateUsdtPerHour ?? 0), body.accruedFrom ?? null, body.accruedTo ?? null, body.source ?? "manual", body.note ?? null, now, now)
        .run();
    const profile = await env.DB.prepare(`SELECT total_reward_usdt, total_reward_super FROM customer_profiles WHERE user_id = ?`)
        .bind(userId)
        .first();
...
async function handleRechargeRecords(env, url) {
    ...
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    const { results } = await env.DB.prepare(`SELECT id, user_id, wallet, pay_token, pay_amount, bnb_amount, status, relay_mode,
...
async function handleWithdrawalRecords(env, url) {
    ...
    if (!source || source === "exchange") {
        ...
        const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
        const { results } = await env.DB.prepare(`SELECT eo.id AS id, eo.user_id AS user_id, eo.wallet AS wallet,
...
async function handleExchangeRecords(env, url) {
    const limit = clampLimit(url.searchParams.get("limit"));
    const userIdFilter = url.searchParams.get("userId")?.trim();
    const walletFilter = url.searchParams.get("wallet")?.trim().toLowerCase();
    const statusFilter = url.searchParams.get("status")?.trim();
...
async function handleFundCollectionRecords(env, url, requesterWallet, requesterRole) {
    ...
    const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
    const { results } = await env.DB.prepare(`SELECT id, requester_wallet, requester_role, source_user_ids_json, source_device_count,
...
async function handleFundCollectionCreate(request, env, requesterWallet, requesterRole, scopeUserId, allowedTypes) {
    await ensureCollectionRequestsTable(env);
    const body = (await request.json().catch(() => null));
    ...
    if (!isEvmAddress(targetWallet))
        return badRequest("targetWallet must be a valid wallet address");
    let amountUsdt = 0;
    let amountSuper = 0;
    ...
    if (acceptedUserIds.length === 0)
        return json({ error: "No accessible users selected" }, 403);
    const now = nowIso();
    const id = createId("fcr");
    await env.DB.prepare(`INSERT INTO fund_collection_requests (
      ...
      target_wallet, amount_usdt, amount_super, status, note, created_at, updated_at
        ...
        .bind(id, requesterWallet.toLowerCase(), requesterRole, JSON.stringify(acceptedUserIds), sourceDeviceCount, targetWallet, amountUsdt.toFixed(6), amountSuper.toFixed(6), body?.note?.trim() || null, now, now)
        .run();
    return json({
        ok: true,
        id,
...
async function handleFundCollectionAction(request, env, requestId, action) {
    await ensureCollectionRequestsTable(env);
    const current = await env.DB.prepare(`SELECT id, status
     FROM fund_collection_requests
     WHERE id = ?`)
        .bind(requestId)
        .first();
    if (!current)
        return json({ error: "Fund collection request not found" }, 404);
    const body = (await request.json().catch(() => null));
    const now = nowIso();
    ...
    if (action === "approve") {
        if (current.status !== "pending")
            return badRequest("Only pending collection requests can be approved");
        await env.DB.prepare(`UPDATE fund_collection_requests
       SET status = 'approved',
           note = COALESCE(?, note),
           updated_at = ?
       WHERE id = ?`)
    ...
    if (action === "reject") {
        if (current.status !== "pending")
            return badRequest("Only pending collection requests can be rejected");
        await env.DB.prepare(`UPDATE fund_collection_requests
       SET status = 'rejected',
           note = COALESCE(?, note),
           updated_at = ?,
    ...
    if (current.status !== "approved")
        return badRequest("Only approved collection requests can be completed");
    await env.DB.prepare(`UPDATE fund_collection_requests
...
async function handleBulkRate(request, env, scopeUserId, allowedTypes) {
    const body = (await request.json().catch(() => null));
...
async function handleMonthlyCardDelete(env, userId, allowedTypes) {
    await ensureProfile(env, userId);
    const service = await applyServiceContractType(env, allowedTypes, userId, "monthly");
    if (!service.ok)
        return service.response;
    const existing = await env.DB.prepare("SELECT contract_end_at, monthly_card_end_at FROM customer_profiles WHERE user_id = ?")
        .bind(userId)
        .first();
...
export async function handleAdmin(request, env, pathParts) {
    ...
    if (!requesterRole)
        return json({ error: "Not admin wallet" }, 403);
    ...
    const isSubAdminCustomerLabelUpdate = !isPrimaryOwner &&
        request.method === "PUT" &&
        pathParts.length === 2 &&
        pathParts[0] === "customers";
    const isSubAdminCustomerDelete = !isPrimaryOwner &&
        request.method === "DELETE" &&
        pathParts.length === 2 &&
        pathParts[0] === "customers";
    const isSubAdminDeviceDelete = !isPrimaryOwner &&
        request.method === "DELETE" &&
        pathParts.length === 2 &&
        pathParts[0] === "devices";
    const isSubAdminCollectionCreate = !isPrimaryOwner &&
        request.method === "POST" &&
        pathParts.length === 1 &&
    ...
    if (request.method === "GET" && pathParts[0] === "records" && pathParts.length === 2) {
        const url = new URL(request.url);
        if (pathParts[1] === "recharges")
            return handleRechargeRecords(env, url);
        if (pathParts[1] === "withdrawals")
            return handleWithdrawalRecords(env, url);
        if (pathParts[1] === "exchanges")
            return handleExchangeRecords(env, url);
    }
