...
export async function writeOwnerAudit(env, entry) {
    ...
    try {
        await env.DB.prepare(`INSERT INTO owner_audit_logs (
        id, actor_wallet, action, target_user_id, target_wallet, payload_json,
        tx_hash, status, error_message, ip, user_agent, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .bind(id, entry.actorWallet.toLowerCase(), entry.action, entry.targetUserId ?? null, entry.targetWallet ? entry.targetWallet.toLowerCase() : null, entry.payload ? JSON.stringify(entry.payload) : null, entry.txHash ?? null, entry.status ?? "ok", entry.error ?? null, ip, ua, now)
            .run();
    }
    catch (err) {
